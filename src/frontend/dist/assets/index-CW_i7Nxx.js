...
function drawPlayerCar(ctx, x, y, w, h, flash) {
...
function drawStartScreen(ctx, cw, ch, time) {
...
function drawGameOver(ctx, gs, time) {
...
function checkCollision(gs, obs) {
...
function initGame(canvas, gs, storedHighScore) {
  ...
  const bgTypes = ["palmtree", "palmtree", "hut", "bush", "crop"];
  gs.bgElements = [];
  for (let i = 0; i < 14; i++) {
    const side = Math.random() > 0.5 ? "left" : "right";
    gs.bgElements.push({
      x: side === "left" ? Math.random() * Math.max(1, gs.roadX - 25) : gs.roadX + gs.roadW + Math.random() * Math.max(1, gs.canvasW - gs.roadX - gs.roadW - 25),
...
function App() {
  ...
  const frameCounterRef = reactExports.useRef(0);
  ...
  const gameLoop = reactExports.useCallback(() => {
    ...
    gs.frameCount++;
    gs.speed = Math.min(12, 3 + gs.frameCount * 22e-4);
    ...
    for (const el of gs.bgElements) {
      ...
      if (el.y > gs.canvasH + 90) {
        ...
        el.x = side === "left" ? Math.random() * Math.max(1, gs.roadX - 25) : gs.roadX + gs.roadW + Math.random() * Math.max(1, gs.canvasW - gs.roadX - gs.roadW - 25);
  ...
  const handleLeftPress = reactExports.useCallback(
    (e) => {
      e.preventDefault();
      const gs = gsRef.current;
  ...
  const handleRightPress = reactExports.useCallback(
    (e) => {
      e.preventDefault();
      const gs = gsRef.current;
      if (gs.state !== "playing") return;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "game-wrapper", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { ref: containerRef, className: "canvas-container", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
      "canvas",
      {
        ref: canvasRef,
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "controls-bar", "data-ocid": "hud.panel", children: [
      ...
      (gameState === "start" || gameState === "gameover") && /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          type: "button",
          className: "play-btn",
          ...
          onClick: startGame,
          children: gameState === "start" ? "🎮 Start Game" : "🔄 Play Again"
        }
      ),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("footer", { className: "game-footer", children: [
      "© ",
